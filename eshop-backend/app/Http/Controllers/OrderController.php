...
class OrderController extends Controller
{
	...
	public function getOrderInfo($id)
	{
		...
		foreach ($orderItems as $item) {
			...
			$temp = ProductVariant::where('id', $item->variant_id)->first()->load('images');
	...
	public function updateOrder()
	{
		...
		if ($status == 'processing') {
			...
			Mail::to($email)->send(new OrderShipped($customerName, $order));
		} else {

			Mail::to($email)->send(new OrderChanged($customerName, $order));
		}
